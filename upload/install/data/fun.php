...
function InstallDefaultData($add){
	...
	mysql_close();
	echo"导入测试数据完毕!<script>self.location.href='index.php?phome=firstadmin&f=5&defaultdata=$add[defaultdata]';</script>";
	exit();
}
//链接数据库
...
function SetDb($add){
	...
	{
		echo"配置数据库完毕!<script>self.location.href='index.php?phome=firstadmin&f=5&defaultdata=$add[defaultdata]';</script>";
	}
	else
	{
		echo"正进入测试数据导入......<script>self.location.href='index.php?phome=defaultdata&f=4&ok=1&defaultdata=$add[defaultdata]';</script>";
	}
	exit();
}
//处理配置文件
function RepEdownConfig($add,$siteurl){
	...
	if(!$fp)
	{
		InstallShowMsg('请检查 /install/data/config.php 文件是否存在!');
	}
	...
	if(!$fp1)
	{
		InstallShowMsg(' /data/config.php 文件权限没有设为0777，配置数据库不成功');
	}
	@fputs($fp1,$data);
	@fclose($fp1);
}
...
function ReturnInstallSql($defaultdata=1){
	...
	if(empty($sql))
	{
		InstallShowMsg(' /install/'.$sqlfile.' 文件丢失,安装不成功','index.php?phome=setdb&f=4');
	}
	return $sql;
}
//取得网站地址
