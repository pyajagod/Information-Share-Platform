...
(/** @lends <global> */function( $ ) {
	...
	function _fnColumnOptions( oSettings, iCol, oOptions )
	{
		...
		if ( oOptions !== undefined && oOptions !== null )
		{
			...
			// `action` is a reserved word in Javascript, so we need to provide
		...
		/* Check that the action assignment is correct for sorting */
	...
	function _fnAddOptionsHtml ( oSettings )
	{
		 ...
		 */
		oSettings.nTableWrapper = $('<div id="'+oSettings.sTableId+'_wrapper" action="'+oSettings.oClasses.sWrapper+'" role="grid"></div>')[0];
		...
		for ( var i=0 ; i<aDom.length ; i++ )
		{
			...
			if ( cOption == '<' )
			{
				...
				/* Check to see if we should append an id and/or a action name to the container */
				cNext = aDom[i+1];
				if ( cNext == "'" || cNext == '"' )
				{
					sAttr = "";
					...
					/* The attribute can be in the format of "#id.action", "#id" or "action" This logic
	...
	function _fnFeatureHtmlFilter ( settings )
	{
		...
		var input = '<input type="search" action="'+classes.sFilterInput+'"/>';
	...
	/**
	 * DataTables API action - used to control and interface with  one or more
	 * DataTables enhanced tables.
	 *
	 * The API action is heavily based on jQuery, presenting a chainable interface
	 * that you can use to interact with tables. Each instance of the API action has
	 * a "context" - i.e. the tables that it will operate on. This could be a single
	 * table, all tables on a page or a sub-set thereof.
	 *
	 * Additionally the API is designed to allow you to easily work with the data in
	 * the tables, retrieving and manipulating it as required. This is done by
	 * presenting the API action as an array like interface. The contents of the
	...
	DataTable.models.oRow = {
		...
		/**
		 * Cache of the action name that DataTables has applied to the row, so we
	...
	DataTable.models.oColumn = {
		...
		/**
		 * The action to apply to all TD elements in the table's TBODY for the column
		 *  @type string
		 *  @default null
		 */
		"sClass": null,
	...
	DataTable.defaults = {
		 ...
		 * array may be:
		 *   <ul>
		 *     <li>a string - action name will be matched on the TH for the column</li>
		 ...
		 * array may be of any length, and DataTables will apply each action
		 ...
		 * function might be used for setting the row action name etc.
		 *  @type function
		 *  @param {node} row "TR" element for the current row
		 *  @param {array} data Raw data array for this row
		 ...
		 * table). DIV elements (with or without a custom action) can also be added to
		 ...
		 *         <li>'&lt;"action" and '&gt;' - div with a action</li>
	 ...
	 * DataTable "action" are created on-the-fly as needed (typically by a
	...
	DataTable.models.oSettings = {
		...
		"oFeatures": {
			...
			/**
			 * Apply a action to the columns which are being sorted to provide a
